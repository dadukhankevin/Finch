...
TASK = Path(__file__).resolve().parents[1] / "benchmarks" / "agentic" / "tasks" / "translation_anomaly"
...
def test_allocator_practice_score_is_hidden_set():
    ...
    assert all("cell " in err or err.startswith("cell") for err in result["errors"]) or result["errors"] == []
...
def test_public_docs_do_not_name_evaluation_languages_or_verses():
    public = (TASK / "CONTRACT.md").read_text() + (TASK / "score.py").read_text()
...
def test_crashing_flag_errors_do_not_name_cells(tmp_path):
    ...
    bad.write_text("def flag(source, candidate, examples):\n    raise RuntimeError('boom')\n")
