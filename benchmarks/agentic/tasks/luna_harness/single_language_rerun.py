#!/usr/bin/env python3
"""Rerun one ordinary staged harness session for stochastic diagnostics."""
...
def main() -> None:
    ...
    evaluation_root = Path(tempfile.mkdtemp(
        prefix=f"finch-luna-rerun-{code}-", dir=work_root
    ))
    try:
        ...
        initial = study.initial_agent_prompt(
            tasks[code][0]["language_name"],
            tasks[code],
            enforce_exploration=True,
            research_only=True,
        )
    ...
    output = {
        "kind": "native_luna_single_language_staged_rerun",
